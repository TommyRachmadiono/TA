...
CREATE TABLE IF NOT EXISTS `mydb`.`like` (
  ...
  `postingan_user_id` INT NOT NULL COMMENT '',
  `user_id` INT NOT NULL COMMENT '',
  PRIMARY KEY (`id`, `postingan_user_id`, `user_id`)  COMMENT '',
  INDEX `fk_postingan_has_user_user1_idx` (`user_id` ASC)  COMMENT '',
  INDEX `fk_postingan_has_user_postingan1_idx` (`id` ASC, `postingan_user_id` ASC)  COMMENT '',
  CONSTRAINT `fk_postingan_has_user_postingan1`
    FOREIGN KEY (`id` , `postingan_user_id`)
    REFERENCES `mydb`.`postingan` (`idpostingan` , `user_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_postingan_has_user_user1`
    FOREIGN KEY (`user_id`)
    REFERENCES `mydb`.`user` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
